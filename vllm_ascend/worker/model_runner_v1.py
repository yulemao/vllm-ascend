...
class NPUModelRunner(GPUModelRunner):
    ...
    def _load_model_edge_cloud(self) -> None:
        ...
        if not (
            ...
            raise NotImplementedError(
                "edge-cloud mode currently supports Qwen3.5, DeepSeek V2/V3/V4, "
                "Kimi-K2.5/K2.6, GLM-4/GLM-5, and MiniMax-M2 models."
            )
    ...
    def _setup_edge_cloud_draft(
        ...
        first_draft_layer = next(iter(predictor.layers.values()))
        is_deepseek_v4_mtp = all(
            hasattr(first_draft_layer, attr)
            for attr in ("e_proj", "h_proj", "hc_mult", "mtp_block")
        )
        ...
        for idx, key in enumerate(layer_keys):
            ...
            if is_deepseek_v4_mtp:
                if is_edge_device() and idx not in local_layers:
                    if not isinstance(layer.mtp_block, PPMissingLayer):
                        layer.mtp_block = PPMissingLayer()
                elif not is_edge_device() and idx in local_layers:
                    # The cloud executes only the decoder block. Input
                    # projections and the vocabulary head remain on the edge.
                    for module_name in (
                        "e_proj",
                        "h_proj",
                        "enorm",
                        ...
                        if module is not None and not isinstance(
                            ...
                            setattr(layer, module_name, PPMissingLayer())
            elif idx not in local_layers and not isinstance(
