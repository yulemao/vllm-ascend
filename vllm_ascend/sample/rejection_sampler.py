...
def rejection_sample(
    ...
    if not sampling_metadata.all_random:
        ...
        if HAS_TRITON:
            ...
            print("[REJ-DBG] pre-sync OK (compute stream)", flush=True)
            rejection_greedy_sample_with_triton(
                output_token_ids,
                num_draft_tokens,
            ...
            print("[REJ-DBG] after kernel, syncing (compute stream)...", flush=True)
            torch.npu.current_stream().synchronize()
            print("[REJ-DBG] post-sync OK (compute stream)", flush=True)
        else:
